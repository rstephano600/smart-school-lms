...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if ($user_id != $_SESSION['user_id']) {
        ...
        if ($stmt->execute()) {
            logActivity($_SESSION['user_id'], 'deleted user', 'user', $user_id);
            echo '<script>showToast("User deleted successfully", "success");</script>';
        }
    }
}
...
if (isset($_GET['restore']) && is_numeric($_GET['restore'])) {
    ...
    $stmt->bind_param("i", $user_id);
    if ($stmt->execute()) {
        logActivity($_SESSION['user_id'], 'restored user', 'user', $user_id);
        echo '<script>showToast("User restored successfully", "success");</script>';
    }
}
                                    ...
                                    <?php if($user['id'] != $_SESSION['user_id']): ?>
                                        <?php if($user['is_active']): ?>
                                            <a href="?delete=<?php echo $user['id']; ?>" 
                                               ...
                                               class="text-red-600 hover:text-red-800" title="Deactivate">
